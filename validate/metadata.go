...
// RequiredFromMetadata enforces the spec.MetadataSource's required markers
// (spec.FieldMeta.Required) at validate/save time, for applications that declare
...
// validator asks the spec.MetadataSource for that field's spec.FieldMeta - using the same
// query convention as the hint panel, spec.FieldMeta(block, "") for a top-level
// block and spec.FieldMeta(block, "source.path") for nested fields - and, when
...
//
// The editor wires the discovered schema and the configured spec.MetadataSource into
// this validator when the session starts; outside editor.Run, or when no
// spec.MetadataSource is configured, it reports nothing.
func RequiredFromMetadata() spec.Validator { return &metadataRuleValidator{check: checkHintRequired} }

// metadataRuleValidator is the shared engine of the FromMetadata validator family.
// It walks the YAML guided by the discovered schema, queries the spec.MetadataSource
// for every field - spec.FieldMeta(block, "") for top-level blocks,
// spec.FieldMeta(block, "a.b") for nested fields, the hint panel's convention -
...
// editor.Run, or without a spec.MetadataSource, the validator is inert.
type metadataRuleValidator struct {
	defs  []schema.FieldDef
	hints spec.MetadataSource
	...
	// report nothing.
	check func(meta spec.FieldMeta, child *yaml.Node, path string, errs *[]spec.Violation)
}

func (v *metadataRuleValidator) Validate(in spec.ValidationInput) []spec.Violation {
...
// where each def is itself a top-level block queried as spec.FieldMeta(name, "");
...
func (v *metadataRuleValidator) walk(node *yaml.Node, defs []schema.FieldDef, blockKey, hintPath, yamlPath string, errs *[]spec.Violation) {
...
// checkHintRequired enforces spec.FieldMeta.Required: absent fields or empty
// scalars violate; a non-scalar value counts as present.
func checkHintRequired(meta spec.FieldMeta, child *yaml.Node, path string, errs *[]spec.Violation) {
	if meta.Required && !yamlnode.PresentNonEmpty(child) {
		*errs = append(*errs, spec.Violation{Path: path, Message: "required"})
	}
}

// OneOfFromMetadata enforces spec.FieldMeta.OneOf from the spec.MetadataSource: a present,
...
func OneOfFromMetadata() spec.Validator { return &metadataRuleValidator{check: checkHintOneOf} }

func checkHintOneOf(meta spec.FieldMeta, child *yaml.Node, path string, errs *[]spec.Violation) {
	if len(meta.OneOf) == 0 {
		return
	}
...
// RangeFromMetadata enforces spec.FieldMeta.Min/Max from the spec.MetadataSource (ValueInRange
...
func RangeFromMetadata() spec.Validator { return &metadataRuleValidator{check: checkHintRange} }

func checkHintRange(meta spec.FieldMeta, child *yaml.Node, path string, errs *[]spec.Violation) {
...
func reportInvalidHintRange(lo, hi, path string, errs *[]spec.Violation) {
	*errs = append(*errs, spec.Violation{
		Path:    path,
...
// PatternFromMetadata enforces spec.FieldMeta.Pattern from the spec.MetadataSource
...
func PatternFromMetadata() spec.Validator {
	...
	return &metadataRuleValidator{check: func(meta spec.FieldMeta, child *yaml.Node, path string, errs *[]spec.Violation) {
...
func checkHintPattern(cache *patternCache, meta spec.FieldMeta, child *yaml.Node, path string, errs *[]spec.Violation) {
	if meta.Pattern == "" {
		return
	}
	re := cache.get(meta.Pattern)
	if re == nil {
		*errs = append(*errs, spec.Violation{Path: path, Message: fmt.Sprintf("invalid pattern %q in hint", meta.Pattern)})
		return
	}
	val, ok := hintScalarValue(child, path, errs)
	if !ok {
...
// CountFromMetadata enforces spec.FieldMeta.MinCount/MaxCount from the spec.MetadataSource
...
func CountFromMetadata() spec.Validator { return &metadataRuleValidator{check: checkHintCount} }

func checkHintCount(meta spec.FieldMeta, child *yaml.Node, path string, errs *[]spec.Violation) {
	...
	if !ok {
		if !isEmptyScalar(child) { // a non-null, non-empty scalar is not a collection
			*errs = append(*errs, spec.Violation{Path: path, Message: "expected a list or mapping"})
			return
		}
...
// UniqueFromMetadata enforces spec.FieldMeta.Unique from the spec.MetadataSource (UniqueValues
// semantics): scalar items in the sequence must not repeat. Non-sequence
// fields and non-scalar items are skipped.
func UniqueFromMetadata() spec.Validator { return &metadataRuleValidator{check: checkHintUnique} }

func checkHintUnique(meta spec.FieldMeta, child *yaml.Node, path string, errs *[]spec.Violation) {
...
// DeprecatedFromMetadata enforces spec.FieldMeta.Deprecated from the spec.MetadataSource
...
func DeprecatedFromMetadata() spec.Validator {
	return &metadataRuleValidator{check: checkHintDeprecated}
}

func checkHintDeprecated(meta spec.FieldMeta, child *yaml.Node, path string, errs *[]spec.Violation) {
	if meta.Deprecated == "" || child == nil {
		return
	}
	*errs = append(*errs, spec.Violation{Path: path, Message: "deprecated - " + meta.Deprecated})
}

// FormatFromMetadata enforces spec.FieldMeta.Formats from the spec.MetadataSource.
...
func FormatFromMetadata() spec.Validator { return &metadataRuleValidator{check: checkHintFormat} }

func checkHintFormat(meta spec.FieldMeta, child *yaml.Node, path string, errs *[]spec.Violation) {
	...
	for _, f := range meta.Formats {
		if !f.IsZero() && f.Matches(child.Value) {
			return
		}
	}
...
// LengthFromMetadata enforces spec.FieldMeta.MinLength/MaxLength from the
// spec.MetadataSource. Length is measured in Unicode code points. A zero value
// for either bound means no rule for that bound.
func LengthFromMetadata() spec.Validator { return &metadataRuleValidator{check: checkHintLength} }

func checkHintLength(meta spec.FieldMeta, child *yaml.Node, path string, errs *[]spec.Violation) {
...
// NotOneOfFromMetadata enforces spec.FieldMeta.NotOneOf from the spec.MetadataSource.
...
// is empty.
func NotOneOfFromMetadata() spec.Validator { return &metadataRuleValidator{check: checkHintNotOneOf} }

func checkHintNotOneOf(meta spec.FieldMeta, child *yaml.Node, path string, errs *[]spec.Violation) {
...
func hintScalarValue(child *yaml.Node, path string, errs *[]spec.Violation) (string, bool) {
	if child == nil {
		return "", false
	}
	if child.Kind != yaml.ScalarNode {
		*errs = append(*errs, spec.Violation{Path: path, Message: "expected a scalar value"})
		return "", false
	}
